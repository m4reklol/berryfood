...
const GalleryInteractive: React.FC<GalleryInteractiveProps> = ({ images, maxVisible = 8 }) => {
  // Gallery state
  ...
  // Simple 2x3 grid layout for 6 images
  const getGridItemClass = (index: number) => {
    // Simple uniform grid - all images same size
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 lg:gap-6">
        {visibleImages.map((image, index) => {
          ...
          return (
            <div
              key={index}
              className={`group relative overflow-hidden bg-gray-100 cursor-pointer transition-all duration-300 hover:shadow-lg hover:shadow-black/10 aspect-[4/3] ${getGridItemClass(index)}`}
